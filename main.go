...
func main() {
	...
	app.Get("/volumes/restore/status", backH.VolumeRestoreStatus)
	app.Post("/volumes/restore", backH.VolumeRestore)
	app.Post("/volumes/remove", compH.GlobalVolumeRemove)
	app.Post("/images/remove", compH.GlobalImageRemove)
	app.Post("/images/prune", compH.GlobalImagePrune)
	...
	app.Post("/containers/prune", compH.GlobalContainerPrune)
	...
	app.Get("/settings/audit-logs", audit.AuditLogsPage(database))
